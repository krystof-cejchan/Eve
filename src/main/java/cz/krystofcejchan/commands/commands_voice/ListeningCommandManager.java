...
public class ListeningCommandManager {
    ...
    public IListeningCommands getCommand(String usersVoiceInput) {
        ...
        try {
            if (UtilityClass.isUserInputVerySimilarToTags(usersVoiceInput) != null)
                return UtilityClass.isUserInputVerySimilarToTags(usersVoiceInput);

            HashMap<String, Double> suitabilityMap = new HashMap<>();
            /*
            ...
            for (IListeningCommands theInterfaceExample : commands) {
                ...
                suitabilityMap.put(theInterfaceExample.getName(), UtilityClass.averageOfDoubleArray(tempResults));
                System.out
