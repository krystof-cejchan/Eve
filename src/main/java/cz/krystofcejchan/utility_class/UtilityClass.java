...
public class UtilityClass {
        ...
        for (IListeningCommands IListeningCmd : new ListeningCommandManager().getAllCommands()) {
            try {
                ...
                if (compareTwoArrays(IListeningCmd.getTags(), words)) {
                    ...
                    map.put(IListeningCmd.getName(), averageOfDoubleArray(tempResults));
                }
     ...
     */
    @Nullable
    public static IListeningCommands getTheMostSuitableIListeningCommandFromAHashMap(HashMap<String, Double> map) {
        IListeningCommands ImostLikelyToBe = null;
        double highest = 0;

        for (IListeningCommands I : new ListeningCommandManager().getAllCommands()) {
            //System.out.println(map);
            if (!map.containsKey(I.getName())) continue;
            if (map.get(I.getName()) > highest) {
                highest = map.get(I.getName());
                ImostLikelyToBe = I;
            }
        }

        return highest >= .45 ? ImostLikelyToBe : null;
    }
